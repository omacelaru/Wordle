...
char cuvant[NUMAR_CUVINTE][NUMAR_LITERE_CUVANT], cuvant_din_combinari[COMBINARI_MAX][NUMAR_LITERE_CUVANT], cuvant_din_produs_car[NUMAR_LITERE_CUVANT], sir_din_solutii[PRODUS_CAR_MAX][NUMAR_LITERE_CUVANT];
...
int cautare_binara(char cuv[], long l, long r) {
    ...
    if (strcmp(cuv, cuvant[m]) == 0)
        return 1;
    else if (strcmp(cuv, cuvant[m]) == 1)
        cautare_binara(cuv, m + 1, r);
    else
        cautare_binara(cuv, l, m - 1);
}
...
int combinari_sucuvant_din_combinariesor() {
...
void combinari_back() {
    ...
    while (a > 0) {
        do {
        } while ((AScombinari = combinari_sucuvant_din_combinariesor()) && !combinari_valid());
        if (AScombinari)
...
int produs_car_valid() {
    ...
    for (i = 0; i < 5; i++)
        if (vector_corect[i] != -1 && b == i + 1) {
            for (j = 0; j < 5; j++) {
                if (cuvant_din_combinari[j][0] == char('A' + vector_corect[i] - 1))
                    x = j;
            }
    ...
    for (i = 0; i < 5; i++) {
        if (vector_incorect[i][0] != -1 && b == i + 1) {
            ...
            for (j = 0; j < numar_col_incorect[i]; j++) {
                for (l = 0; l < 5; l++)
                    if (cuvant_din_combinari[l][0] == char('A' + vector_incorect[i][j] - 1)) {
                        v[j] = l;
                        nr++;
                    }
            }
...
int generare() {
    ...
    for (int i = 0; i < numar_solutie_combinari && !gasit_cuvant; i++) {
        for (int j = 0; j < 5; j++) {
            cuvant_din_combinari[j][0] = litera[solutie_combinari[i][j] - 1].lit[0];
            cuvant_din_combinari[j][1] = '\0';
        }
        numar_solutie_produs_car = 0;
        produs_car_back();
        numar_solutii = 0;
        for (int k = 0; k < numar_solutie_produs_car; k++) {
            cuvant_din_produs_car[0] = cuvant_din_combinari[solutie_produs_car[k][0] - 1][0];
            cuvant_din_produs_car[1] = cuvant_din_combinari[solutie_produs_car[k][1] - 1][0];
            cuvant_din_produs_car[2] = cuvant_din_combinari[solutie_produs_car[k][2] - 1][0];
    ...
    for (int re = 0; re < 5 && poz1 < poz2; re++) {
        ...
        lmax = -1;
        do {
            car = int(sir_din_solutii[i][lit]);
            stg = i + 1;
            while (stg <= j && car == int(sir_din_solutii[stg][lit]))
                stg++;
            l = stg - i;
            i = stg;
            if (lmax < l) {
...
void verificare() {
    ...
    strcpy(cuvant_gasit, sir_din_solutii[index_cuvant]);
    cuvant_gasit[5] = '\0';
